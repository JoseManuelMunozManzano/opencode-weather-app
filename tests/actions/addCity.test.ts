...
describe('handleAdd', () => {
  test('agrega ciudad y fija default cuando no existe', async () => {
    ...
    restoreFetch = stubFetch(
      () =>
        new Response(
          JSON.stringify({
            results: [{ name: 'Ottawa', country: 'Canadá', admin1: 'Ontario', latitude: 45.41, longitude: -75.69 }],
          }),
          { status: 200 }
        )
    );
  ...
  test('duplicada no se agrega dos veces', async () => {
    ...
    restoreFetch = stubFetch(
      () =>
        new Response(JSON.stringify({ results: [{ name: 'Madrid', latitude: 40.41, longitude: -3.7 }] }), {
          status: 200,
        })
    );
